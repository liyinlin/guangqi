    ...
    MobileArea.prototype = {
        ...
        bindEvent: function() {
            ...
            function popupArea(e) {
                ...
                $(".area_roll>div").hide();
                $(".area_roll>div").eq(_self.indexs).show();
                areaCtrlInit();
            ...
            function areaCtrlInit() {
                var data=_self.valueTo.attr("data");
                var value=[0,0,0];
                if(data){
                    value=data.split(",");
                    for(var i=0;i<value.length;i++){
                        _self.value[i]=parseInt(value[i]);
                    }
                }
            ...
            _self.getData(function() {
                _self.trigger.on('click', popupArea);
            });
        },
        //重置节点个数
        ...
        finish: function(e) {
            ...
            _self.city = [provinceText,cityText,countyText];
            _self.values=_self.city[_self.indexs];
            _self.valueTo.html(_self.values);
            var datas=_self.valueTo.attr("src");
            var data=provinceVal + ((cityVal)?(',' + cityVal):(',' + 0))+ ((countyVal)?(',' + countyVal):(',' + 0));
            _self.valueTo.attr("data",data);
            if(_self.valueTo2){
                _self.valueTo2.html(_self.city[1]);
                _self.valueTo2.attr("data",data);
            }
            _self.close(e);
        },
